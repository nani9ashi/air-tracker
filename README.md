...
- **有料版**: カスタムサイクル / 複数自転車 / 履歴無制限 / ヒートマップ全期間 / 広告非表示。
...
|---|---|---|
| 自転車 | 1台 | 複数台（追加・切替） |
| 推奨サイクル | プリセット（7/14/21/28日） | ＋カスタム日数 |
| 履歴表示 | 直近30件 | 全件 |
| ヒートマップ | 直近1ヶ月 | 全期間 |
| 広告 | 表示（将来） | 非表示（将来） |

> **課金は未実装。** `settings.isPremium` は現状 false 固定で、有料機能は**ロック表示（ガード）のみ**。
> データは非破壊：履歴・既存の複数自転車は保存され続け、解放時にそのまま使える。
> 上限は `src/store/store.js` の `FREE_LIMITS` に集約。広告は未実装のため「将来」。
