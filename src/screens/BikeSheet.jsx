...
import { setActiveBike, addBike, FREE_LIMITS } from '../store/store.js'
...
export default function BikeSheet({ open, onClose }) {
  ...
  const addLocked = !state.settings.isPremium && bikes.length >= FREE_LIMITS.bikes
  ...
  return (
    <Sheet open={open} onClose={onClose} title="自転車を選ぶ">
      ...
      >
        {addLocked ? (
          <>
            <Icon name="lock" size={16} /> 自転車を追加（プレミアム）
          </>
        ) : (
          '＋ 自転車を追加'
        )}
      ...
      {showPremium && (
        <p className="bike-sheet__premium" role="status">
          <Icon name="lock" size={14} /> 複数の自転車の管理はプレミアムで解放されます
        </p>
      )}
    </Sheet>
  )
